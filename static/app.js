document.addEventListener('DOMContentLoaded', () => {
    ...
    window.triggerTool = (tool) => {
        const promptParams = promptUserForTool(tool);
        if (!promptParams) return;
        ...
        fetch('/api/tools/execute', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ tool, params: promptParams })
        })
        .then(res => res.json())
        .then(data => {
            if (tool === 'search') {
            ...
            } else if (tool === 'ksearch') {
                const summary = data.results.join('\n\n---\n\n');
                addMessage('assistant', `Knowledge search results:\n${summary}`);
            } else if (tool === 'code') {
    ...
    function promptUserForTool(tool) {
        ...
        if (tool === 'improve') {
            const target = prompt("Target tool file:", "tools/web_search.py");
    ...
    saveSettings.addEventListener('click', () => {
        ...
        const updateTopics = fetch('/api/autonomous/topics', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ topics })
        });

        const updateModel = fetch('/api/model/update_id', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ model_id })
        });

        Promise.all([updateTopics, updateModel])
        .then(() => {
